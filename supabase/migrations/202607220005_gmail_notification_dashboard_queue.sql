-- Route major notification email through the connected ARK ONE Google mailbox
-- and correct live ticket queue counts for legacy blank assignment fields.
...
begin
  ...
  if actor_role = 'engineer' then
    select
      count(*) filter (where lower(coalesce(t.status, '')) not in ('closed','completed','approved','resolved')),
      count(*) filter (where lower(coalesce(t.status, '')) in ('pending_review','submitted')),
      count(*) filter (where lower(coalesce(t.status, '')) in ('closed','completed','approved','resolved')
    ...
    select
      count(*) filter (where lower(coalesce(status, '')) not in ('closed','completed','approved','resolved')),
      count(*) filter (where lower(coalesce(status, '')) in ('pending_review','submitted')),
      count(*) filter (where lower(coalesce(status, '')) in ('closed','completed','approved','resolved')
...
comment on function public.ark_department_dashboard_live_summary() is
  'Role-scoped live dashboard summary with normalized ticket queue assignments.';
