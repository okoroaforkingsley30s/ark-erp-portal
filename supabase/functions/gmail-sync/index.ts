...
async function getAccessToken(supabase: any, connection: any) {
  ...
  if (!response.ok || !tokens.access_token) throw new Error('Unable to refresh Gmail access')
...
serve(async (req) => {
  ...
  try {
    ...
    if (!listResponse.ok) return jsonResponse({ error: 'Gmail message list failed' }, 502)
    ...
    if (rows.length) {
      ...
      if (saveError) {
        ...
        return jsonResponse({ error: 'Messages were fetched but could not be saved' }, 500)
      }
    }
    ...
    return jsonResponse({ error: 'Unable to synchronize Gmail' }, 500)
  }
})
