...
import { canUserAccess, getRoleHome, ROLE_LABELS } from '@/lib/roleAccess';
  ...
  <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#08153d] via-[#0b1f5e] to-[#102969] p-6 text-white">
    <div className="max-w-lg rounded-2xl border border-red-400/30 bg-[#102969]/90 p-6 shadow-xl text-center">
      ...
      <p className="mt-2 text-sm text-slate-300">
        Role: <span className="font-semibold text-white">{ROLE_LABELS[user?.role] || user?.role || 'Unknown'}</span>
      </p>
      <p className="text-xs text-slate-400 mt-1">Required permission: {permission}</p>
      <a href={`#${getRoleHome(user?.role)}`} className="mt-5 inline-flex rounded-md bg-[#ff5a00] px-4 py-2 text-sm font-semibold text-white hover:bg-[#e24f00]">
        Go to my dashboard
      </a>
    </div>
  </div>
...
export default function ProtectedRoute({
  ...
  if (permission && !canUserAccess(user, permission)) {
    return <AccessDenied user={user} permission={permission} from={location.pathname} />;
  }
